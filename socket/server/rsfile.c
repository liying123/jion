...
#define NAME_UNIQID_PATH "/home/ly/server/namefile"
...
/* 函数功能：保存需要注册的不同的文件的name、uniqid到指定文件的接口
     返回值：0--正常文件； 606--获取配置信息失败；610--入参异常； 611--文件操作异常； 
             612--空文件； 613--status码异常； 614--文件格式都合法但是md5不对； 其他值--status码值 */
...
int name_uniqid(const char* pname,char* puniqid)
{
    ...
    */
    ...
    for(Index = 0;Index < 1;Index++)
    {
        ...
        //if(name_uniqid_get_impl(name,uniqid) != 0)
        if(name_uniqid_get_impl(pname,puniqid) != 0)
        {
            printf("%s:get failed,return.\n",__FUNCTION__);
            return -1;
        }
        //printf("%s:direct get mutex pname:%s,uniqid:%s\n\n",__FUNCTION__,pname,puniqid);
