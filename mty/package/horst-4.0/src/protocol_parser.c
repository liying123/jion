...
{
	int olen, whlen, llclen, iplen;
	
	bedo_log(BEDO_LOG_DEBUG, "parse_packet begin....\n");	
	olen = len;
	...
	if (len == 0)
		...
		return 0;
	whlen = olen - len;
	bedo_log(BEDO_LOG_DEBUG, "buffer len=%d, wlan_len=%d", olen, whlen);

	len = parse_llc(&buf, len, p);
	if (len <= 0)
		return 1;
	llclen = olen - whlen - len;	
	bedo_log(BEDO_LOG_DEBUG, "llc_len=%d", llclen);

	len = parse_ip_header(&buf, len, p);
	if (len <= 0) {
		if (len == 0) {
			if ((p->pkt_types&PKT_TYPE_TCP) == PKT_TYPE_TCP) {
				char ipsrc[32], ipdst[32];	
				strcpy(ipsrc, ip_sprintf(p->ip_src));
				strcpy(ipdst, ip_sprintf(p->ip_dst));
				bedo_log(BEDO_LOG_INFO, "buffer len=%d, wlan_len=%d, llc_len=%d, srcip=%s, dstip=%s", 
						olen, whlen, llclen, ipsrc, ipdst);
			}
		}
		return 1;
	}
	iplen = olen - whlen - llclen;
	bedo_log(BEDO_LOG_DEBUG, "iplen=%d", iplen);
	bedo_log(BEDO_LOG_DEBUG, "buffer len=%d, wlan_len=%d, llc_len=%d, ip_len=%d", olen, whlen, llclen, iplen);
...
parse_llc(unsigned char ** buf, int len, struct packet_info* p)
{
	...
	*buf = *buf + 6;
	...
	if (ntohs(*((uint16_t*)*buf)) == 0x4305) {
		...
		if (**buf != 0x08) {
			if (**buf != 0x88/*802.1X Authentication*/)
				return -1;
			(*buf)++;
			if (**buf == 0x8e)
				p->pkt_types |= PKT_TYPE_HANDSHAKE;
			return 0;
		}
		(*buf)++;
		if (**buf == 0x06) { /* ARP */
