...
struct timeval the_send_time;		/* cycle judgment send hmac and server_ip time */
struct timeval the_serveripr0_time;	/* read statictype_ip-dhcp_ip clear time in r0 */
struct timeval the_macipr1_time;	/* hmac r1 clear time */
struct timeval the_macipr3_time;	/* hmac r3 clear time */

int R = 0; 	/* if install in R0 assignment 0 to R;if install in R1 assignment 1 to R;
		 * if install in R3 assignment 3 to R */
char card[] = "R0";	/* R0 R1 R3 represent different small card */

static unsigned char macbuf[21];
static unsigned char ip[15];
static unsigned char mac_ip[38];

int sockfd_1, sockfd_3; /*UDP socket for send mac and server_ip to R1,R3 synchronous*/
struct sockaddr_in hostaddr_1;
struct sockaddr_in hostaddr_3;
...
int sockfd; /*UDP socket for send collect_dta to R0 host-trans*/
...
static bool virtual_acc_record(const char *type, const char *account, const char *mac, struct packet_info* p)
{
	...
	sprintf(buf, "%d, %s, %s, %s", tv.tv_sec, type, mac_smbig_conversion(mac), account);
	fprintf(VF, buf);
	fprintf(VF, "\n");
	fflush(VF);
...
dump_mac_cache(struct packet_info* p)
{
	...
	if (ret == HMAP_S_OK) {
		...
		sprintf(buf, "%s\t%s\t%s\t%d\t%d\t%d\t%c\t%s\t%s\t%d\t%s\t%d\t%d\t%c\t%s\t%c\t%c\t%s\r\n", mac_smbig_conversion(ether_sprintf(p->wlan_src)), 
	...
	if (ret == HMAP_S_OK) {
			...
			gettimeofday(&the_time, NULL);
			sprintf(buf, "%s\t%s\t%s\t%d\t%d\t%d\t%c\t%s\t%s\t%d\t%s\t%d\t%d\t%c\t%s\t%c\t%c\t%s\r\n", mac_smbig_conversion(ether_sprintf(p->wlan_src)), 
				phone_brand(ether_sprintf(p->wlan_dst)), essid, the_time.tv_sec, sig, id, f, p->wlan_essid, mac_smbig_conversion(ether_sprintf(p->wlan_dst)), 
				p->wlan_channel, des_type(p), X, Y, f, macbuf, f, f, card);
			t_content.type = type_1;
			t_content.length = strlen(buf);
			strcpy(t_content.buff, buf);
...
/* R0 R1 R3 horst send collect_data to R0  host-trans  */
int
udp_cli(void)
{
...
/* only when install in R0 perform, create connection to R1  */
int
udp_cli_1(void)
{
	if((sockfd_1 = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
		err(1,"Could not open local socket");
	printf("udp_1 cli sockufd is:%d\n", sockfd_1);

	bzero(&hostaddr_1, sizeof(hostaddr_1));
	hostaddr_1.sin_family = AF_INET;
	hostaddr_1.sin_addr.s_addr = inet_addr("192.168.123.1");//htonl(INADDR_ANY);
	hostaddr_1.sin_port = htons(atoi("5555"));

	if(connect(sockfd_1, (struct sockaddr *)&hostaddr_1, sizeof(hostaddr_1)) < 0)
	{	
		err(1,"connect failed");
	}
	else
	{
		printf("udp_1 client connect success\n");
	}
	return sockfd_1;
}

/* only when install in R0 perform, send hmac and server_ip to R1  */
void
send_hmac_ip_tor1(void)
{
	sprintf(mac_ip, "%s:%s", macbuf, ip);
	printf("send mac_ip:%s,%d\n", mac_ip, strlen(mac_ip));
	sendto(sockfd_1, mac_ip, 38, 0, (struct sockaddr *)&hostaddr_1, sizeof(hostaddr_1));
	gettimeofday(&the_macipr1_time, NULL);
}

/* only when install in R0 perform, create connection to R3  */
int
udp_cli_3(void)
{
	if((sockfd_3 = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
		err(1,"Could not open local socket");
	printf("udp_3 cli sockufd is:%d\n", sockfd_3);

	bzero(&hostaddr_3, sizeof(hostaddr_3));
	hostaddr_3.sin_family = AF_INET;
	hostaddr_3.sin_addr.s_addr = inet_addr("192.168.123.3");//htonl(INADDR_ANY);
	hostaddr_3.sin_port = htons(atoi("5555"));

	if(connect(sockfd_3, (struct sockaddr *)&hostaddr_3, sizeof(hostaddr_3)) < 0)
	{	
		err(1,"connect failed");
	}
	else
	{
		printf("udp_3 client connect success\n");
	}
	return sockfd_3;
}

/* only when install in R0 perform, send hmac and server_ip to R3 */
void
send_hmac_ip_tor3(void)
{
	sprintf(mac_ip, "%s:%s", macbuf, ip);
	printf("send mac_ip:%s,%d\n", mac_ip, strlen(mac_ip));
	sendto(sockfd_3, mac_ip, 38, 0, (struct sockaddr *)&hostaddr_3, sizeof(hostaddr_3));
	gettimeofday(&the_macipr3_time, NULL);	
}

/* replace server_ip to the configuration file ~~ dhcp——ip */
void
replace_server_ip(const char dhcpip[15])
{
	static char buf[256];
	memset(buf, 0, 256);
	int len = 0, len1 = 0;
	...
	/* replace server ip to the configuration file -- dhcp  */
	FILE *fp = fopen("/etc/config/dhcp", "r+");
	if (fp == NULL) {
		printf("Open dhcp ip failed\n");
	}
	while(fgets(buf, sizeof(buf), fp)) {
		len = len + strlen(buf);
		printf("read dhcp buf:%s,%d\n", buf, strlen(buf));
		if(strstr(buf, "ip '") != NULL) {
			printf("read dhcp ip buf:%s,%d\n", buf, strlen(buf));
			len1 = len - strlen(buf);
			res = fseek(fp, len1, SEEK_SET);
			if (res < 0)
				perror("fseek failed");
...
/* only when install R1 or R3 perform, read R1 or R3 dhcp_ip */
void
card_judge_serverip(void)
{
	static char buf[256], *pbuf, *ppbuf;
	static unsigned dhcpip[15];
	int len;
	...
	while(fgets(buf, sizeof(buf), fp)) {
		...
		if(NULL != strstr(buf, "ip")) {
			...
			ppbuf = trim(ppbuf);
			len = strlen(ppbuf);
			strncpy(dhcpip, ppbuf, len);
			printf("read server_ip dhcpip:%s,%d\n", dhcpip, strlen(dhcpip));
		}
                bzero(buf, sizeof(buf));
        }
        fclose(fp);

	if (strcmp(ip, dhcpip) != 0) {
		replace_server_ip(dhcpip);
	}
	else if (strcmp(ip, dhcpip) == 0) {
		printf("The receive_ip and read dhcp_ip is same!\n");
	}
}

/* only when install R1 or R3 perform, synchronous R0 hmac to R1 and R3 /opt/scanner/hmac */
void
hmac_operation(const char buf[21])
{
	FILE *fhmac, *fhmac1, *fhmac2;

	if ((fhmac = fopen("/opt/scanner/hmac", "r")) == NULL) {
		printf("Open hmac r failed!\n");
	}
	fread(macbuf, 21, 1, fhmac);
	printf("Before write,read hamc macbuf, len:%s, %d\n", macbuf, strlen(macbuf));
	fclose(fhmac);

	if (strcmp(buf, macbuf) != 0) {
		if ((fhmac1 = fopen("/opt/scanner/hmac", "w")) == NULL) {
			printf("Open hmac w failed!\n");
		}
		fwrite(buf, 21, 1, fhmac1);
		fclose(fhmac1);

		if ((fhmac2 = fopen("/opt/scanner/hmac", "r")) == NULL) {
			printf("Open hmac r failed!\n");
		}
		fread(macbuf, 21, 1, fhmac2);
		printf("After write,read hamc macbuf, len:%s, %d\n", macbuf, strlen(macbuf));
		fclose(fhmac2);
	}
	else if (strcmp(buf, macbuf) == 0) {
		printf("The receive_hmac and read /opt/scanner/hmac is same!\n");
	}
}

/* only install in R1 perform, receive hmac and server_ip from R0 */
void 
udp_socket_server_1(void)
{
	static char buf[40], rmac[21], *pbuf;
	memset(buf, 0 ,40);
	memset(rmac, 0 ,21);
	int sockurfd_1;
	struct sockaddr_in hostaddr_1, cliaddr_1;
	socklen_t peerlen;
	peerlen = sizeof(cliaddr_1);
	FILE *fhmac;

	if ((sockurfd_1 = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
		err(1, "Could not create local socket");
	printf("Udp server R1 received hmac from R0  is : %d\n",sockurfd_1);

	bzero(&hostaddr_1, sizeof(struct sockaddr_in));
	hostaddr_1.sin_family = AF_INET;
	hostaddr_1.sin_addr.s_addr = htonl(INADDR_ANY);//inet_addr("192.168.1.1");
	hostaddr_1.sin_port = htons(atoi("5555"));
	if (bind(sockurfd_1, (struct sockaddr *)&hostaddr_1, sizeof(hostaddr_1)) < 0)
		err(1, "bind failed");

	if (recvfrom(sockurfd_1, buf, 40, 0, (struct sockaddr*)&cliaddr_1, &peerlen) > 0) {
		printf("recvfrom mac_ip:%s,%d\n", buf, strlen(buf));
		pbuf = buf;

		strncpy(rmac, strsep(&pbuf, ":"), 21);
		printf("receive macbuf hmac and len:%s,%d\n", rmac, strlen(rmac));
		hmac_operation(rmac);

		strncpy(ip, trim(pbuf),strlen(trim(pbuf)));
		printf("receive server_ip and len:%s,%d\n", ip, strlen(ip));
		card_judge_serverip();
...
/* only install in R3 perform, receive hmac and server_ip from R0  */
void 
udp_socket_server_3(void)
{
	static char buf[40], rmac[21], *pbuf;
	memset(buf, 0 ,40);
	memset(rmac, 0 ,21);
	int sockurfd_3;
	struct sockaddr_in hostaddr_3, cliaddr_3;
	socklen_t peerlen;
	peerlen = sizeof(cliaddr_3);
	FILE *fhmac;

	if ((sockurfd_3 = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
		err(1, "Could not create local socket");
	printf("Udp server R3 received mac from R0  is : %d\n",sockurfd_3);

	bzero(&hostaddr_3, sizeof(struct sockaddr_in));
	hostaddr_3.sin_family = AF_INET;
	hostaddr_3.sin_addr.s_addr = htonl(INADDR_ANY);//inet_addr("192.168.1.1");
	hostaddr_3.sin_port = htons(atoi("5555"));
	if (bind(sockurfd_3, (struct sockaddr *)&hostaddr_3, sizeof(hostaddr_3)) < 0)
		err(1, "bind failed");

	if (recvfrom(sockurfd_3, buf, 40, 0, (struct sockaddr*)&cliaddr_3, &peerlen) > 0) {
		printf("recvfrom mac_ip:%s,%d\n", buf, strlen(buf));
		pbuf = buf;
...
/* only when install R0 perform */
void
r0_judge_serverip(void)
{
	static char buf[256], *pbuf, *ppbuf;
	static char buf1[256], *pbuf1, *ppbuf1;
	static unsigned dhcpip[15];
	memset(buf, 0, 256);
	memset(buf1, 0, 256);
	memset(dhcpip, 0, 15);

	/* Read the IP in the configuration file -- statictype */
	FILE *fp = fopen("/etc/config/statictype", "r");
	if (fp == NULL) {
		printf("Open statictype server_ip failed\n");
	}
	while(fgets(buf, sizeof(buf), fp)) {
		pbuf = buf;
		if(NULL != strstr(buf, "server_ip")) {
			...
			ppbuf = strsep(&pbuf, "'");
			ppbuf = trim(ppbuf);
			strncpy(ip, ppbuf, strlen(ppbuf));
		}
                bzero(buf, sizeof(buf));
        }
        fclose(fp);

	/* Read the IP in the configuration file -- dhcp  */
	FILE *fp1 = fopen("/etc/config/dhcp", "r");
	if (fp1 == NULL) {
		printf("Open dhcp ip failed\n");
	}
	while(fgets(buf1, sizeof(buf1), fp1)) {
		pbuf1 = buf1;
		if(NULL != strstr(buf1, "ip")) {
			strsep(&pbuf1, "'");
			ppbuf1 = strsep(&pbuf1, "'");
			ppbuf1 = trim(ppbuf1);
			strncpy(dhcpip, ppbuf1, strlen(ppbuf1));
			printf("read server_ip dhcpip:%s,%d\n", dhcpip, strlen(dhcpip));
		}
	...
	if (strcmp(ip, dhcpip) != 0){
		...
		printf("The read statictype_ip and read dhcp_ip is sname!\n");
	}

	gettimeofday(&the_serveripr0_time, NULL);	
}
...
main(int argc, char** argv)
{
	...
	if (R == 0) {
		...
		printf("main macbuf:%s, %d\n", macbuf, strlen(macbuf));

		r0_judge_serverip();
		printf("main read statictype ip:%s %d\n", ip, strlen(ip));
		...
		/*  If the installation in the R1 perform this function  */
		udp_socket_server_1();
	}
	else if (R == 3) {
		/*  If the installation in the R3 perform this function  */
		udp_socket_server_3();
	}
	...
	{
		if (R == 0) {
			gettimeofday(&the_send_time, NULL);
			if (the_send_time.tv_sec - the_serveripr0_time.tv_sec >= 10) {
				r0_judge_serverip();
			}

			if (the_send_time.tv_sec - the_macipr1_time.tv_sec >= 30*60) {
				send_hmac_ip_tor1();
			}
			if (the_send_time.tv_sec - the_macipr3_time.tv_sec >= 30*60) {
				send_hmac_ip_tor3();
			}
		}
