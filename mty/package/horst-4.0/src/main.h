...
#define PKT_TYPE_HANDSHAKE	0x100000
...
#define PKT_TYPE_ALL		(PKT_TYPE_CTRL | PKT_TYPE_MGMT | PKT_TYPE_DATA | PKT_TYPE_ALL_MGMT | PKT_TYPE_ALL_CTRL | PKT_TYPE_ALL_DATA | PKT_TYPE_BADFCS | PKT_TYPE_HANDSHAKE)
...
#define MAPPKG_INITIAL_SIZE 512

typedef struct macpkg_t {
        unsigned short          wlan_type;      /* frame control field */
...
struct config {
	...
	int			channel_ini;	/* args set channel_idx*/
	int			display_interval;
	char*			dumpfile;
	char*			bedologfile;	/* bedo log file */
	char*			viraccfile;	/* virtual account(SNS+IM) file */
	int			printlog;	/* print log or not */
	int			capture;	/* capture TCP packet */
	int			handshake;	/* capture handshake */
	int			automatchchannel; /* 0: not auto match. 1:match strongest signal AP. 2:match assigned ssid */
	char			assign_ssid[WLAN_MAX_SSID_LEN];
	int			upload_interval; /* in seconds */
	...
	int			paused;
	int			num_channels;
	int 			avoid_repeat_time;
};

struct around_ap_info
{
	char    essid[WLAN_MAX_SSID_LEN];
	int     signal;
	int     channel;
	bool    encrypt;
	struct around_ap_info *next;
};

struct file_cache_info 
{
	int	rfn;		/* read_file_no */
	int	wfn;		/* write_file_no */
	bool	reading;
	int	maxcount; 
};

struct curl_readcb_info
{
	char	*pMem;
	long 	size;
};
...
enum _bedo_log_level {
	BEDO_LOG_DEBUG = 0,
	BEDO_LOG_INFO = 1,
	BEDO_LOG_WARN = 2, 
	BEDO_LOG_ERROR = 3
};
typedef unsigned char bedo_log_level;
