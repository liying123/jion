...
static pthread_t upload_mac_thd, upload_sns_thd;
static pthread_mutex_t t_sns_lock, t_mac_lock; 
...
static void
dump_sns_cache(const char *type, const char *account, const char *mac);
...
taobao_filter(const unsigned char *buffer, int buflen, struct packet_info* p, const char* wlan_src, const char* wlan_dst)
{
	...
	for (i=0; i<sizeof(taobao_flags)/sizeof(char*); i++) {
		...
		dump_sns_cache("TAOBAO", account, bindmac);
		break;
	}
...
get_options(int argc, char** argv)
{
	...
	while((c = getopt(argc, argv, "hqsCPi:I:t:c:p:e:f:d:o:b:X::x:m:u:U:a:l:v:A:")) > 0) {
		switch (c) {
		case 'p':
			conf.port = optarg;
			break;
...
upload_mac_fun(void *arg)
{
	...
	for (;;) {		
		...
		if (mac_cache_info.reading == true && fpsize(cachefile) > 0) {
			...
			char *cmd = malloc(512);
			sprintf(cmd, "/opt/scanner/upload.sh mac %s", cachefile);	
			system(cmd);
			free(cmd);
...
dump_sns_cache(const char *type, const char *account, const char *mac)
{
	...
	virtual_acc_record(type, account, mac);		
}
